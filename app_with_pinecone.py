...
def initialize_pinecone():
    ...
    try:
        ...
        # FIX 2: k=6 se zyada context milega (pehle k=3 tha)
...
def add_pdf_to_vectorstore(pdf_path):
    ...
    try:
        ...
        # FIX 3: Chunk size badha diya — zyada context per chunk
        ...
        # FIX 4: Source metadata clean — filename nahi, sirf content
        for chunk in chunks:
            chunk.metadata['source'] = Path(pdf_path).stem  # extension hatao
...
# FIX 1: Naya prompt — PDF mention nahi karega, detailed response dega
rag_prompt_template = """You are a knowledgeable and empathetic medical information assistant.
...
🚨 CRITICAL RULES:
1. EMERGENCIES (chest pain, severe bleeding, difficulty breathing, loss of consciousness):
   → Immediately say: "⚠️ MEDICAL EMERGENCY! Call 911/108 IMMEDIATELY!"

2. You CANNOT diagnose or prescribe — always say "Please consult a doctor"

3. Respond in the SAME LANGUAGE as the user's question (English or Hindi)

RESPONSE STYLE:
- Write as a knowledgeable doctor explaining to a patient — clear, warm, thorough
- Give COMPREHENSIVE, well-structured information
- Use sections like: Overview | Symptoms | Causes | Diagnosis | Treatment | Prevention
- Use bullet points for lists
- Aim for 250-400 words — be genuinely helpful, not superficial
- DO NOT mention any PDF file names, document names, or "according to [source]"
- DO NOT say things like "as per the document" or "the factsheet says"
- Write as if YOU know this information — no citations, no file references
...
def get_bot_response(user_question):
    ...
    try:
        if retriever and vectorstore:
            ...
            if docs:
                # FIX 5: Context mein source nahi daalenge — sirf content
                context = "\n\n---\n\n".join([
                    doc.page_content for doc in docs
                ])
            else:
                context = "No specific documents found. Use your general medical knowledge."
        else:
            context = "Vector store not initialized. Use your general medical knowledge."
